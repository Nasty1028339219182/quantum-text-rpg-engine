"""Built-in UI strings (not game content). Game text lives in YAML."""

from __future__ import annotations

STRINGS: dict[str, dict[str, str]] = {
    "prompt": {"ru": "> ", "en": "> "},
    "hp": {"ru": "ОЗ", "en": "HP"},
    "mp": {"ru": "ОМ", "en": "MP"},
    "gold": {"ru": "Золото", "en": "Gold"},
    "xp": {"ru": "Опыт", "en": "XP"},
    "level": {"ru": "Уровень", "en": "Level"},
    "exits": {"ru": "Выходы", "en": "Exits"},
    "items_here": {"ru": "Предметы", "en": "Items"},
    "people_here": {"ru": "Здесь", "en": "Here"},
    "containers": {"ru": "Ёмкости", "en": "Containers"},
    "locked": {"ru": "заперто", "en": "locked"},
    "hidden": {"ru": "скрыто", "en": "hidden"},
    "nothing": {"ru": "Ничего особенного.", "en": "Nothing special."},
    "unknown": {
        "ru": "Не понял. Набери «помощь» для списка команд.",
        "en": "I don't understand. Type 'help' for commands.",
    },
    "gone": {"ru": "Этого здесь нет.", "en": "That isn't here."},
    "no_item": {"ru": "У тебя нет такого предмета.", "en": "You don't have that."},
    "cant_take": {"ru": "Это нельзя взять.", "en": "You can't take that."},
    "taken": {"ru": "Ты берёшь: {name}.", "en": "You take the {name}."},
    "dropped": {"ru": "Ты кладёшь: {name}.", "en": "You drop the {name}."},
    "already_have": {"ru": "У тебя это уже есть.", "en": "You already have that."},
    "inv_full": {"ru": "Инвентарь полон.", "en": "Inventory is full."},
    "too_heavy": {"ru": "Слишком тяжело.", "en": "That's too heavy."},
    "inventory": {"ru": "Инвентарь", "en": "Inventory"},
    "empty_inv": {"ru": "Пусто.", "en": "Empty."},
    "cat_all": {"ru": "Все", "en": "All"},
    "cat_weapon": {"ru": "Оружие", "en": "Weapons"},
    "cat_armor": {"ru": "Броня", "en": "Armor"},
    "cat_shield": {"ru": "Щиты", "en": "Shields"},
    "cat_accessory": {"ru": "Украшения", "en": "Trinkets"},
    "cat_consumable": {"ru": "Расходники", "en": "Supplies"},
    "cat_key": {"ru": "Ключи", "en": "Keys"},
    "cat_book": {"ru": "Книги", "en": "Books"},
    "cat_quest": {"ru": "Задания", "en": "Quest items"},
    "cat_misc": {"ru": "Прочее", "en": "Other"},
    "cat_unknown": {"ru": "Такой вкладки нет. Например: инвентарь оружие.", "en": "No such tab. Example: inventory weapon."},
    "equipped": {"ru": "надето", "en": "equipped"},
    "equip_ok": {"ru": "Ты надеваешь: {name}.", "en": "You equip the {name}."},
    "unequip_ok": {"ru": "Ты снимаешь: {name}.", "en": "You unequip the {name}."},
    "cant_equip": {"ru": "Это нельзя надеть.", "en": "You can't equip that."},
    "used": {"ru": "Ты используешь: {name}.", "en": "You use the {name}."},
    "cant_use": {"ru": "Не получается использовать это так.", "en": "You can't use that that way."},
    "no_exit": {"ru": "Туда не пройти.", "en": "You can't go that way."},
    "need_key": {"ru": "Заперто. Нужен ключ.", "en": "Locked. You need a key."},
    "unlocked": {"ru": "Открыто.", "en": "Unlocked."},
    "dark": {
        "ru": "Здесь слишком темно. Нужен источник света.",
        "en": "It's too dark. You need a light source.",
    },
    "you_are": {"ru": "Ты", "en": "You"},
    "stats": {"ru": "Характеристики", "en": "Stats"},
    "quests": {"ru": "Задания", "en": "Quests"},
    "no_quests": {"ru": "Пока нет заданий.", "en": "No quests yet."},
    "quest_active": {"ru": "активно", "en": "active"},
    "quest_done": {"ru": "выполнено", "en": "done"},
    "quest_failed": {"ru": "провалено", "en": "failed"},
    "quest_now": {"ru": "Сейчас: {text}", "en": "Now: {text}"},
    "quest_hours": {"ru": "Осталось часов: {n}.", "en": "Hours left: {n}."},
    "quest_expired": {"ru": "Срок вышел.", "en": "The time ran out."},
    "level_pick": {"ru": "Уровень {level}. Выбери одно.", "en": "Level {level}. Choose one."},
    "ability_learned": {"ru": "Новое умение: {name}.", "en": "New ability: {name}."},
    "rumors": {"ru": "Слухи", "en": "Rumors"},
    "saved": {"ru": "Игра сохранена: {slot}", "en": "Game saved: {slot}"},
    "loaded": {"ru": "Игра загружена: {slot}", "en": "Game loaded: {slot}"},
    "no_save": {"ru": "Сохранение не найдено.", "en": "Save not found."},
    "bye": {"ru": "До встречи.", "en": "Goodbye."},
    "dead": {"ru": "Ты погибаешь.", "en": "You die."},
    "win": {"ru": "*** ПОБЕДА ***", "en": "*** VICTORY ***"},
    "lose": {"ru": "*** КОНЕЦ ***", "en": "*** GAME OVER ***"},
    "combat": {"ru": "БОЙ", "en": "COMBAT"},
    "attack": {"ru": "Атаковать", "en": "Attack"},
    "defend": {"ru": "Защищаться", "en": "Defend"},
    "flee": {"ru": "Бежать", "en": "Flee"},
    "use_item": {"ru": "Предмет", "en": "Item"},
    "hit": {"ru": "{who} наносит {dmg} урона {target}.", "en": "{who} hits {target} for {dmg}."},
    "miss": {"ru": "{who} промахивается.", "en": "{who} misses."},
    "enemy_down": {"ru": "{name} падает.", "en": "{name} falls."},
    "you_hit": {"ru": "Ты наносишь {dmg} урона ({name}).", "en": "You deal {dmg} damage ({name})."},
    "you_miss": {"ru": "Промах.", "en": "Miss."},
    "got_xp": {"ru": "Опыт +{xp}.", "en": "+{xp} XP."},
    "got_loot": {"ru": "Добыча: {name}.", "en": "Loot: {name}."},
    "fled": {"ru": "Ты сбегаешь.", "en": "You flee."},
    "cant_flee": {"ru": "Не удалось сбежать.", "en": "You fail to flee."},
    "defending": {"ru": "Ты принимаешь защитную стойку.", "en": "You take a defensive stance."},
    "no_target": {"ru": "Некого атаковать.", "en": "No one to attack."},
    "already_dead": {"ru": "Уже мертво.", "en": "Already dead."},
    "talk_nobody": {"ru": "Здесь не с кем говорить.", "en": "No one to talk to."},
    "who": {"ru": "С кем? Уточни имя.", "en": "Who? Specify a name."},
    "what": {"ru": "Что именно?", "en": "What exactly?"},
    "search_fail": {"ru": "Ты ничего не находишь.", "en": "You find nothing."},
    "search_ok": {"ru": "Ты обыскиваешь место.", "en": "You search the area."},
    "skill_ok": {"ru": "Успех ({roll} против {dc}).", "en": "Success ({roll} vs {dc})."},
    "skill_fail": {"ru": "Неудача ({roll} против {dc}).", "en": "Failure ({roll} vs {dc})."},
    "healed": {"ru": "Восстановлено {n} ОЗ.", "en": "Healed {n} HP."},
    "damaged": {"ru": "Потеряно {n} ОЗ.", "en": "Lost {n} HP."},
    "gold_gain": {"ru": "Золото +{n}.", "en": "+{n} gold."},
    "gold_lose": {"ru": "Золото −{n}.", "en": "−{n} gold."},
    "not_enough_gold": {"ru": "Не хватает золота.", "en": "Not enough gold."},
    "bought": {"ru": "Куплено: {name} за {n}.", "en": "Bought {name} for {n}."},
    "sold": {"ru": "Продано: {name} за {n}.", "en": "Sold {name} for {n}."},
    "shop": {"ru": "Лавка", "en": "Shop"},
    "shop_empty": {"ru": "Нечего предложить.", "en": "Nothing for sale."},
    "shop_hint": {
        "ru": "Команды: купить <предмет>, продать <предмет>, уйти.",
        "en": "Commands: buy <item>, sell <item>, leave.",
    },
    "craft_ok": {"ru": "Создано: {name}.", "en": "Crafted: {name}."},
    "craft_fail": {"ru": "Не хватает ингредиентов.", "en": "Missing ingredients."},
    "no_recipe": {"ru": "Такого рецепта нет.", "en": "No such recipe."},
    "opened": {"ru": "Открыто: {name}.", "en": "Opened: {name}."},
    "need_lockpick": {"ru": "Не открывается.", "en": "It won't open."},
    "rest_ok": {"ru": "Ты отдыхаешь. Силы возвращаются.", "en": "You rest. Strength returns."},
    "cant_rest": {"ru": "Здесь нельзя отдыхать.", "en": "You can't rest here."},
    "journal": {"ru": "Журнал", "en": "Journal"},
    "map": {"ru": "Известные места", "en": "Known places"},
    "roads": {"ru": "Дороги", "en": "Roads"},
    "meters": {"ru": "Состояние", "en": "Condition"},
    "hunger": {"ru": "Голод", "en": "Hunger"},
    "map_hidden": {"ru": "Этого этажа ты ещё не видел.", "en": "You have not seen that floor."},
    "no_floor": {"ru": "Нет такого этажа и нет такого хода.", "en": "No such floor and no such step."},
    "noted": {"ru": "Пометка: {text}", "en": "Note: {text}"},
    "notes": {"ru": "Пометки", "en": "Notes"},
    "scenes": {"ru": "Сцены", "en": "Scenes"},
    "no_scene": {"ru": "Нет такой сцены.", "en": "No such scene."},
    "check_ok": {"ru": "Всё сходится.", "en": "All of it holds."},
    "check_bad": {"ru": "Не сходится: {n}.", "en": "Does not hold: {n}."},
    "no_regions": {"ru": "Отсюда нет дальней дороги.", "en": "No long road leaves here."},
    "no_road": {"ru": "Такой дороги нет.", "en": "There is no such road."},
    "travel": {"ru": "Дорога в {name}. Часов в пути: {hours}.", "en": "The road to {name}. Hours: {hours}."},
    "lang_set": {"ru": "Язык: русский.", "en": "Language: English."},
    "help_title": {"ru": "Команды", "en": "Commands"},
    "help_body": {
        "ru": (
            "осмотреться, идти <направление>, взять <предмет>, положить <предмет>,\n"
            "осмотреть <что>, инвентарь, надеть / снять <предмет>, использовать <предмет>,\n"
            "говорить <кто>, атаковать <кто>, искать, открыть <что>, читать <что>,\n"
            "купить / продать, создать <рецепт>, дать <предмет> <кому>,\n"
            "характеристики, задания, журнал, карта, ехать, отряд, приказ, отдохнуть, ждать,\n"
            "сохранить [слот], загрузить [слот], язык ru|en, звук, громкость, помощь, выход"
        ),
        "en": (
            "look, go <dir>, take <item>, drop <item>, examine <thing>,\n"
            "inventory, equip / unequip <item>, use <item>, talk <who>,\n"
            "attack <who>, search, open <thing>, read <thing>,\n"
            "buy / sell, craft <recipe>, give <item> <who>,\n"
            "stats, quests, journal, map, travel, party, order, rest, wait,\n"
            "save [slot], load [slot], language ru|en, sound, volume, help, quit"
        ),
    },
    "dirs": {"ru": "направления: север юг восток запад вверх вниз", "en": "dirs: north south east west up down"},
    "status": {"ru": "Эффекты", "en": "Effects"},
    "weight": {"ru": "Вес", "en": "Weight"},
    "ac": {"ru": "Защита", "en": "AC"},
    "weapon": {"ru": "Оружие", "en": "Weapon"},
    "armor": {"ru": "Броня", "en": "Armor"},
    "level_up": {"ru": "Уровень повышен! ({level})", "en": "Level up! ({level})"},
    "gave": {"ru": "Ты отдаёшь {name}.", "en": "You give the {name}."},
    "wait": {"ru": "Время идёт.", "en": "Time passes."},
    "read": {"ru": "{name}:\n{text}", "en": "{name}:\n{text}"},
    "no_read": {"ru": "Тут нечего читать.", "en": "Nothing to read."},
    "combined": {"ru": "Ты соединяешь предметы. Получается: {name}.", "en": "You combine them into: {name}."},
    "cant_combine": {"ru": "Эти предметы не соединяются.", "en": "Those don't combine."},
    "which_item": {"ru": "Какой предмет? {list}", "en": "Which item? {list}"},
    "combat_menu": {
        "ru": "1 атаковать  2 предмет  3 защищаться  4 бежать",
        "en": "1 attack  2 item  3 defend  4 flee",
    },
    "pick_enemy": {"ru": "Кого? {list}", "en": "Whom? {list}"},
    "poison_tick": {"ru": "Яд снимает {n} ОЗ.", "en": "Poison deals {n} HP."},
    "effect_end": {"ru": "Эффект закончился: {name}.", "en": "Effect ended: {name}."},
    "say_what": {"ru": "Что сказать?", "en": "Say what?"},
    "nothing_happens": {"ru": "Ничего не происходит.", "en": "Nothing happens."},
    "ambiguous": {"ru": "Уточни: {list}", "en": "Be more specific: {list}"},
    "visited": {"ru": "(было)", "en": "(visited)"},
    "light_needed": {"ru": "Нужен свет.", "en": "You need light."},
    "broke_lock": {"ru": "Ты взламываешь запор.", "en": "You break the lock."},
    "left": {"ru": "Ты уходишь.", "en": "You leave."},
    "gui_leave": {"ru": "Уйти", "en": "Leave"},
    "gui_play": {"ru": "Играть", "en": "Play"},
    "gui_open": {"ru": "Открыть папку…", "en": "Open folder…"},
    "gui_quit": {"ru": "Выход", "en": "Quit"},
    "gui_save": {"ru": "Сохранить", "en": "Save"},
    "gui_load": {"ru": "Загрузить", "en": "Load"},
    "gui_menu": {"ru": "В меню", "en": "Menu"},
    "gui_games": {"ru": "Игры", "en": "Games"},
    "gui_hint": {
        "ru": "Выбери игру и нажми «Играть». «Редактор» правит её. «Новая игра» создаёт свою. Седой Брод, Тень крепости и Первые шаги уже в списке.",
        "en": "Buttons on the right are actions. You can also type a command below.",
    },
    "gui_type": {"ru": "команда", "en": "command"},
    "gui_send": {"ru": "Ввод", "en": "Enter"},
    "gui_look": {"ru": "Осмотреться", "en": "Look"},
    "gui_search": {"ru": "Искать", "en": "Search"},
    "gui_rest": {"ru": "Отдохнуть", "en": "Rest"},
    "gui_talk": {"ru": "Говорить", "en": "Talk"},
    "gui_attack": {"ru": "Атаковать", "en": "Attack"},
    "gui_take": {"ru": "Взять", "en": "Take"},
    "gui_examine": {"ru": "Осмотреть", "en": "Examine"},
    "gui_use": {"ru": "Использовать", "en": "Use"},
    "gui_equip": {"ru": "Надеть", "en": "Equip"},
    "gui_drop": {"ru": "Бросить", "en": "Drop"},
    "gui_sell": {"ru": "Продать", "en": "Sell"},
    "gui_open_c": {"ru": "Открыть", "en": "Open"},
    "gui_craft": {"ru": "Создать", "en": "Craft"},
    "gui_actions": {"ru": "Действия", "en": "Actions"},
    "gui_waiting": {"ru": "Ждёт ввода", "en": "Waiting for input"},
    "gui_busy": {"ru": "…", "en": "…"},
    "gui_slot": {"ru": "Имя слота:", "en": "Slot name:"},
    "gui_no_saves": {"ru": "Нет сохранений.", "en": "No saves."},
    "gui_pick_game": {"ru": "Выбери игру", "en": "Choose a game"},
    "gui_subtitle": {
        "ru": "Текстовый RPG-движок. Без графики.",
        "en": "A text RPG engine. No graphics.",
    },
    "gui_ended": {"ru": "Игра окончена", "en": "Game over"},
    "gui_shop_npc": {"ru": "Лавка", "en": "Shop"},
    "ed_title": {"ru": "Редактор", "en": "Editor"},
    "ed_open": {"ru": "Редактор", "en": "Editor"},
    "ed_new": {"ru": "Новая игра", "en": "New game"},
    "ed_new_name": {"ru": "Название игры:", "en": "Game name:"},
    "ed_new_id": {"ru": "Имя латиницей, без пробелов. Игрок его не видит. Например: inn", "en": "A latin name, no spaces. The player never sees it. Example: inn"},
    "ed_add": {"ru": "добавить", "en": "add"},
    "ed_game": {"ru": "Об игре", "en": "About the game"},
    "ed_rooms": {"ru": "Комнаты", "en": "Rooms"},
    "ed_items": {"ru": "Вещи", "en": "Things"},
    "ed_npcs": {"ru": "Люди", "en": "People"},
    "ed_dialogues": {"ru": "Разговоры", "en": "Talk"},
    "ed_quests": {"ru": "Задания", "en": "Quests"},
    "ed_fights": {"ru": "Враги", "en": "Fights"},
    "ed_recipes": {"ru": "Рецепты", "en": "Recipes"},
    "ed_abilities": {"ru": "Умения", "en": "Abilities"},
    "ed_loot": {"ru": "Добыча", "en": "Loot"},
    "ed_events": {"ru": "События", "en": "Events"},
    "ed_hooks": {"ru": "Свой код", "en": "Your code"},
    "ed_nav_start": {"ru": "С чего начать", "en": "Start here"},
    "ed_nav_world": {"ru": "Мир", "en": "World"},
    "ed_nav_story": {"ru": "Сюжет", "en": "Story"},
    "ed_nav_fight": {"ru": "Бой", "en": "Fight"},
    "ed_nav_extra": {"ru": "Своё", "en": "Yours"},
    "ed_more": {"ru": "Ещё", "en": "More"},
    "ed_back": {"ru": "К списку", "en": "Back to the list"},
    "ed_delete_btn": {"ru": "Удалить", "en": "Delete"},
    "ed_new_room": {"ru": "Новая комната", "en": "New room"},
    "ed_new_item": {"ru": "Новая вещь", "en": "New thing"},
    "ed_new_npc": {"ru": "Новый человек", "en": "New person"},
    "ed_new_talk": {"ru": "Новый разговор", "en": "New talk"},
    "ed_new_quest": {"ru": "Новое задание", "en": "New quest"},
    "ed_new_fight": {"ru": "Новый враг", "en": "New fight"},
    "ed_new_recipe": {"ru": "Новый рецепт", "en": "New recipe"},
    "ed_new_ability": {"ru": "Новое умение", "en": "New ability"},
    "ed_new_loot": {"ru": "Новая добыча", "en": "New loot"},
    "ed_empty": {"ru": "Пока пусто.", "en": "Nothing here yet."},
    "ed_guide_title": {"ru": "С чего начать", "en": "Start here"},
    "ed_guide_body": {
        "ru": "Игра — это текст. Слева разделы, справа то, что ты правишь. Не нужно заполнять всё сразу: пустая игра уже запускается.",
        "en": "A game is text. Sections are on the left, the thing you are editing is on the right. You do not have to fill everything. An empty game already runs.",
    },
    "ed_guide_1": {"ru": "1. Об игре — название и комната, где герой появляется.", "en": "1. About the game — the title, and the room where the hero starts."},
    "ed_guide_2": {"ru": "2. Комнаты — места. У каждой есть текст и выходы в другие комнаты.", "en": "2. Rooms — places. Each has text and exits to other rooms."},
    "ed_guide_3": {"ru": "3. Вещи и люди — что можно взять и с кем говорить.", "en": "3. Things and people — what can be taken, and who can be talked to."},
    "ed_guide_4": {"ru": "4. «Играть» сверху справа. Так видно, что получилось. «Проверить» скажет, если выход ведёт в никуда.", "en": "4. Play, at the top right, shows the result. Check tells you if an exit leads nowhere."},
    "ed_hint_game": {"ru": "Название, герой и первая комната. Остальное можно не трогать.", "en": "Title, hero, and the first room. The rest can wait."},
    "ed_hint_rooms": {"ru": "Места, где ходит герой. Сначала текст комнаты, потом выход в другую.", "en": "Places the hero walks. Write the room, then an exit to another."},
    "ed_hint_items": {"ru": "Девять вкладок: оружие, броня, щит и остальные. Новая вещь попадает во вкладку, которая открыта.", "en": "Nine tabs: weapon, armor, shield, and the rest. A new thing is filed under the open tab."},
    "ed_hint_npcs": {"ru": "Кто стоит в комнате. Лавка и спутник тоже здесь.", "en": "Who stands in a room. A shop and a companion live here too."},
    "ed_hint_dialogues": {"ru": "Что человек говорит и какие есть ответы.", "en": "What a person says, and the answers."},
    "ed_hint_quests": {"ru": "Задание: зачем идти и когда оно кончается.", "en": "A quest: why to go, and when it ends."},
    "ed_hint_fights": {"ru": "Враг. Имя, здоровье, удар.", "en": "An enemy. Name, health, blow."},
    "ed_hint_recipes": {"ru": "Из чего что получается.", "en": "What is made from what."},
    "ed_hint_abilities": {"ru": "Особый удар в бою. Тратит силы.", "en": "A special blow in a fight. It spends strength."},
    "ed_hint_loot": {"ru": "Что может выпасть. Шанс — число от 1 до 100.", "en": "What may drop. Chance is a number from 1 to 100."},
    "ed_hint_events": {"ru": "Короткий текст, который срабатывает сам. Можно оставить пустым.", "en": "A short text that fires on its own. It can stay empty."},
    "ed_hint_hooks": {"ru": "Код на Python, если YAML уже не хватает. Новой игре это не нужно.", "en": "Python, when YAML is not enough. A new game does not need this."},
    "ed_id_note": {"ru": "Имя латиницей, без пробелов. Игрок его не видит. Например: inn", "en": "A latin name, no spaces. The player never sees it. Example: inn"},
    "ed_validate": {"ru": "Проверить", "en": "Validate"},
    "ed_play_here": {"ru": "Отсюда", "en": "From here"},
    "ed_pick_room": {"ru": "Сначала открой комнату. Потом «Ещё» и «Отсюда».", "en": "Open a room first. Then More, and From here."},
    "ed_package": {"ru": "Собрать", "en": "Package"},
    "ed_packaged": {"ru": "Собрано: {path}", "en": "Packed: {path}"},
    "ed_saved": {"ru": "Сохранено: {path}", "en": "Saved: {path}"},
    "ed_copied": {
        "ru": "Игра из комплекта скопирована для правки:\n{path}",
        "en": "Bundled game copied for editing:\n{path}",
    },
    "ed_bad_id": {"ru": "Такой id нельзя или он уже есть.", "en": "Bad or duplicate id."},
    "ed_delete": {"ru": "Удалить {id}?", "en": "Delete {id}?"},
    "ed_unsaved": {
        "ru": "Есть несохранённые правки. Выйти?",
        "en": "Unsaved changes. Leave anyway?",
    },
    "ed_unsaved_mark": {"ru": "не сохранено", "en": "unsaved"},
    "ed_status_ok": {"ru": "Правки остаются в памяти, пока не нажмёшь «Сохранить».", "en": "Edits stay in memory until you press Save."},
    "ed_more_fields": {"ru": "Остальное. Для первой игры не нужно.", "en": "The rest. A first game does not need it."},
    "pick_class": {"ru": "Выбери класс:", "en": "Choose a class:"},
    "class_set": {"ru": "Класс: {name}.", "en": "Class: {name}."},
    "no_mp": {"ru": "Не хватает сил.", "en": "Not enough strength."},
    "time_shift": {"ru": "Время сдвинулось. Сейчас {phase}.", "en": "Time moved on. It is {phase}."},
    "phase_night": {"ru": "ночь", "en": "night"},
    "phase_morning": {"ru": "утро", "en": "morning"},
    "phase_day": {"ru": "день", "en": "day"},
    "phase_evening": {"ru": "вечер", "en": "evening"},
    "shop_closed": {"ru": "Лавка закрыта. Сейчас {phase}.", "en": "The stall is shut. It is {phase}."},
    "shop_rep": {"ru": "{name}: {n}. Цена от этого.", "en": "{name}: {n}. The price follows that."},
    "npc_leaves": {"ru": "{name} уходит.", "en": "{name} leaves."},
    "npc_arrives": {"ru": "{name} приходит.", "en": "{name} arrives."},
    "follows_now": {"ru": "{name} идёт с тобой.", "en": "{name} walks with you."},
    "follows": {"ru": "С тобой: {names}.", "en": "With you: {names}."},
    "order_who": {"ru": "Кому? приказ <имя> жди | за мной | не дерись", "en": "Who? order <name> wait | follow | hold"},
    "order_wait": {"ru": "{name} ждёт: {place}.", "en": "{name} waits: {place}."},
    "order_wait_short": {"ru": "ждёт: {place}", "en": "waits: {place}"},
    "order_follow": {"ru": "{name} идёт за тобой.", "en": "{name} follows you."},
    "order_follow_short": {"ru": "за тобой", "en": "follows"},
    "order_hold": {"ru": "{name} идёт за тобой и не лезет в бой.", "en": "{name} follows and stays out of the fight."},
    "order_hold_short": {"ru": "не лезет в бой", "en": "out of the fight"},
    "order_catchup": {"ru": "{name} догоняет.", "en": "{name} catches up."},
    "order_wait_btn": {"ru": "жди", "en": "wait"},
    "order_follow_btn": {"ru": "за мной", "en": "follow"},
    "order_hold_btn": {"ru": "не дерись", "en": "hold"},
    "with_you": {"ru": "с тобой", "en": "with you"},
    "with_you_down": {"ru": "с тобой, лежит", "en": "with you, down"},
    "ally_hit": {"ru": "{name} бьёт {target} (−{dmg}).", "en": "{name} hits {target} (−{dmg})."},
    "ally_miss": {"ru": "{name} промахивается.", "en": "{name} misses."},
    "ally_hurt": {"ru": "{who} бьёт {name} (−{dmg}).", "en": "{who} hits {name} (−{dmg})."},
    "ally_evade": {"ru": "{name} уклоняется от {who}.", "en": "{name} dodges {who}."},
    "ally_down": {"ru": "{name} падает и не встаёт до отдыха.", "en": "{name} drops and will not rise until you rest."},
    "ally_down_short": {"ru": "лежит", "en": "down"},
    "ally_up": {"ru": "Снова на ногах: {names}.", "en": "On their feet again: {names}."},
    "party": {"ru": "Отряд", "en": "Party"},
    "party_empty": {"ru": "Ты один.", "en": "You are alone."},
    "party_full": {"ru": "В отряде уже {n}.", "en": "The party is already {n}."},
    "reputation": {"ru": "Репутация", "en": "Reputation"},
    "rep_change": {"ru": "Репутация ({name}): {n:+d}.", "en": "Reputation ({name}): {n:+d}."},
    "rep_set": {"ru": "Репутация ({name}) теперь {n}.", "en": "Reputation ({name}) is now {n}."},
    "hungry": {"ru": "Голод скручивает живот (−{n}).", "en": "Hunger twists your gut (−{n})."},
    "starved": {"ru": "Ты умираешь от голода.", "en": "You starve."},
    "sated": {"ru": "Голод спадает. Сейчас {n}.", "en": "The hunger eases. It is {n}."},
    "sound_on": {"ru": "Звук включён.", "en": "Sound on."},
    "sound_off": {"ru": "Звук выключен.", "en": "Sound off."},
    "sound_btn": {"ru": "Звук", "en": "Sound"},
    "volume_now": {"ru": "Музыка {music}, эффекты {sfx}.", "en": "Music {music}, effects {sfx}."},
    "ed_library": {"ru": "Библиотека", "en": "Library"},
    "ed_help": {"ru": "Справка", "en": "Help"},
    "ed_import": {"ru": "Вставить", "en": "Insert"},
    "ed_imported": {"ru": "Вставлено: {id}", "en": "Inserted: {id}"},
    "ed_lib_empty": {"ru": "Библиотека пуста.", "en": "Library is empty."},
}

DIRS = {
    "north": {
        "id": "north",
        "ru": "север",
        "en": "north",
        "aliases": ["n", "с", "север", "north", "на север"],
    },
    "south": {
        "id": "south",
        "ru": "юг",
        "en": "south",
        "aliases": ["s", "ю", "юг", "south", "на юг"],
    },
    "east": {
        "id": "east",
        "ru": "восток",
        "en": "east",
        "aliases": ["e", "в", "восток", "east", "на восток"],
    },
    "west": {
        "id": "west",
        "ru": "запад",
        "en": "west",
        "aliases": ["w", "з", "запад", "west", "на запад"],
    },
    "up": {
        "id": "up",
        "ru": "вверх",
        "en": "up",
        "aliases": ["u", "вверх", "наверх", "up", "вверх"],
    },
    "down": {
        "id": "down",
        "ru": "вниз",
        "en": "down",
        "aliases": ["d", "вниз", "down", "вниз"],
    },
}


def t(lang: str, key: str, **kwargs) -> str:
    block = STRINGS.get(key, {})
    text = block.get(lang) or block.get("en") or block.get("ru") or key
    if kwargs:
        try:
            return text.format(**kwargs)
        except (KeyError, IndexError, ValueError):
            return text
    return text


def dir_id(token: str) -> str | None:
    tok = (token or "").strip().lower()
    if not tok:
        return None
    for did, info in DIRS.items():
        aliases = [did, info["ru"], info["en"], *info.get("aliases", [])]
        if tok in {a.lower() for a in aliases}:
            return did
    return None


def dir_name(did: str, lang: str) -> str:
    info = DIRS.get(did)
    if not info:
        return did
    return info.get(lang) or info.get("en") or did
